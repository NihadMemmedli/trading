...
| `split_definitions` | id, dataset_id, feature_set_id, name, split_type, split_hash, config_json, created_at | unique dataset/feature_set/name/split_hash; JSONB config; index dataset/feature_set and split_hash |
| `split_windows` | split_definition_id, window_index, split_name, start_at, end_at, decision_time, created_at | train/validation/test rows; check start/end/decision ordering; unique split/window/name; cascade from split definition |
| `model_experiments` | id, dataset_id, feature_set_id, split_definition_id, name, model_name, parameter_hash, experiment_hash, code_version, parameters_json, metrics_json, status, timestamps | JSONB parameters/metrics; status checks; index dataset/feature_set, split_definition, status/created_at, and experiment_hash |
